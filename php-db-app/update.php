...
if (isset($_GET['id'])) {
    try {
        ...
        // vendersテーブルからvender_codeカラムのデータを取得するためのSQL文を変数$sql_select_vender_codesに代入する
        $sql_select_vender_codes = 'SELECT vender_code FROM venders';

        // SQL文を実行する
        $stmt_select_vender_codes = $pdo->query($sql_select_vender_codes);

        // SQL文の実行結果を配列で取得する
        // 補足：PDO::FETCH_COLUMNは1つのカラムの値を1次元配列（多次元ではない普通の配列）で取得する設定である
        $vender_codes = $stmt_select_vender_codes->fetchAll(PDO::FETCH_COLUMN);
            ...
            <form action="update.php?id=<?= $_GET['id'] ?>" method="post" class="registration-form">
                <div>
                    ...
                    <select id="vender_code" name="vender_code" required>
                        ...
                        <?php
                        // 配列の中身を順番に取り出し、セレクトボクスの選択肢として出力する
                        foreach ($vender_codes as $vender_code) {
                            // もし変数$vender_codeが商品の仕入先コードの値と一致していれば、selected属性をつけて初期値にする
                            if ($vender_code === $product['vender_code']) {
                                echo "<option value='{$vender_code}' selected>{$vender_code}</option>";
                            } else {
