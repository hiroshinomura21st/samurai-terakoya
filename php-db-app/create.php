...
if (isset($_POST['submit'])) {
    try {
        ...
        $sql_insert = '
        INSERT INTO products (product_code, product_name, price, stock_quantity, vender_code)
        VALUES (:product_code, :product_name, :price, :stock_quantity, :vender_code)
    ';
    $stmt_insert = $pdo->prepare($sql_insert);

    // bindValue()メソッドを使って実際に値をプレースホルダにバインドする（割り当てる）
    ...
    $stmt_insert->bindValue(':vender_code', $_POST['vender_code'], PDO::PARAM_INT);
...
try {
    ...
    // verndersテーブルからvender_codekカラムのデータを取得するためのSQL文を変数$sql_selectに代入する
    $sql_select = 'SELECT vender_code FROM venders';
    ...
    $vender_codes = $stmt_select->fetchAll(PDO::FETCH_COLUMN);
} catch (PDOException $e) {
    exit($e->getMessage());
}
?>
...
<head>
    ...
    <!-- Google Fontsの読み込み -->
     ...
     <link href="https://fonts.gooleapis.com/css?family=Noto+Sans+JP&display=swap" rel="stylesheet">
</head>
        ...
        <article class="registration">
            ...
            <form action="create.php" method="post" class="registration-form">
                <div>
                    ...
                    <select id="vender_code" name="vender_code" required>
                        <option disabled selected value>選択してください</option>
                        <?php
                        // 配列の中身を順番に取り出し、セレクトボックスの選択肢として出力する
                        foreach ($vender_codes as $vender_code) {
                            echo "<option value='{$vender_code}'>{$vender_code}</option>";
                        }
                        ?>
                    </select>
                </div>
